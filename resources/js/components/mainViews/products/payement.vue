<template>
  <div>
    <div id="Checkout" class="inline" v-if="!paid && addNewCreditCard">
      <h1>Pay Invoice</h1>
      <div class="text-left m-1">
        <button
          @click="verifyMe()"
          class="btn p-0 m-0 py-3 px-3 btn-transparent fw-bold text-warning"
        >
          Back to shipping address
        </button>
      </div>


      <div class="formDiv mt-3">
        <div class="form-group">
          <label for="PaymentAmount">Payment amount</label>
          <div class="amount-placeholder">
            <span>$</span>
            <span>{{ total.toFixed(2) }}</span>
          </div>
        </div>
        <div class="form-group py-2">
          <label or="NameOnCard">Name on card</label>
          <input
            id="NameOnCard"
            v-model="creditCard.nameOfCard"
            class="form-control"
            type="text"
            maxlength="30"
          />

          <span v-if="errors.nameOfCard" class="p-0 text-danger">
            {{ errors.nameOfCard[0] }}
          </span>
        </div>
        <div class="form-group py-2">
          <label for="CreditCardNumber">Card number</label>
          <input
            id="CreditCardNumber"
            @change="detectCardType(creditCard.cardNumber)"
            @keypress="onlyNumber"
            v-model="creditCard.cardNumber "
            class="null card-image form-control"
            type="text"
            v-cardformat:formatCardNumber
          />

          <span v-if="errors.cardNumber" class="p-0 text-danger">
            {{ errors.cardNumber[0] }}
          </span>
        </div>
        <div class="expiry-date-group form-group py-2">
          <label for="ExpiryDate">Expiry date</label>
          <input
            id="ExpiryDate"
            class="form-control"
            v-model="creditCard.cardExpDate"
            type="text"
            placeholder="MMYY"
            maxlength="9"
            @keypress="onlyNumber"
            @input="updateValue2($event)"
          />
          <span v-if="errors.cardExpDate" class="p-0 py-2 text-danger">
            {{ errors.cardExpDate[0] }}
          </span>
        </div>

        <div class="security-code-group form-group py-2">
          <label for="SecurityCode">CVV</label>
          <div class="input-container">
            <input
              id="SecurityCode"
              v-cardformat:formatCardCVC
              v-model="creditCard.cardCvvCode"
              class="form-control"
              type="text"

            @keypress="onlyNumber"

            />
            <i id="cvc" class="fa fa-question-circle"></i>
          </div>
          <span v-if="errors.cardCvvCode" class="p-0 pt-2 text-danger">
            {{ errors.cardCvvCode[0] }}
          </span>

          <div class="cvc-preview-container pt-4 two-card hide">
            <div class="amex-cvc-preview"></div>
            <div class="visa-mc-dis-cvc-preview"></div>
          </div>
        </div>

        <button
          @click="addCreditCard"
          id="PayButton"
          style="width: 100%; margin-top: 28px"
          class="btn btn-block btn-success"
        >
          <span class="submit-button-lock"></span>
          <span class="align-middle">Pay ${{ total.toFixed(2) }}</span>
        </button>
      </div>
    </div>

    <div class="inline creditCards" v-if="!paid && !addNewCreditCard">
      <div class="text-center">
           <h3>Amount : ${{total.toFixed(2)}}</h3>
        <small>Please select a credit card</small>
      </div>
      <div class="row p-0 m-0">
        <div
          class="col-lg-6 col-md-6 col-sm-12 py-5"
          style="display: flex; justify-content: center"
          v-for="(creditCard, key) in creditCards"
          :key="key"
        >
          <div
            class="wrapper"
            style="cursor: pointer"
            @click="setCreditCardInfo(key)"
          >
            <div class="cc visa">
              <svg width="295" height="87">
                <path d="M 0 0 C 50 50 250 0 300 87"></path>
              </svg>
              <div class="container">
                <div class="type">
                  {{ creditCard.cardType.toUpperCase() }}
                </div>
                <!-- <div class="circuit">
       <i class="fab fa-cc-visa fa-2x"></i>
      </div> -->
              </div>
              <div class="holder">
                <span class="name">{{ creditCard.nameOfCard }}</span>
                <span class="number">{{
                  creditCardDashedNumber[key]
                }}</span>


                <span>{{
                  creditCard.cardExpDate

                }}</span>
              </div>
            </div>
          </div>
        </div>

        <div
          class="selectCard3dBtns mt-5"
          style="display: flex; justify-content: space-around"
        >
          <button
            v-on:click="setStepCart()"
            class="btn rounded-pill text-white py-1 px-3 btn-dark"
          >
            Back to cart
          </button>

          <button
            @click='resetCreditCard()'
            v-on:click="addNewCreditCard = true"
            class="btn rounded-pill text-white py-1 px-3 btn-warning"
          >
            New payment card
          </button>
        </div>
      </div>
    </div>

    <div
      v-if="paid"
      style="display: flex; align-items: center; height: 80vh"
      :key="conRender"
    >
      <div class="funds-success-message-container">
        <div class="funds-checkmark-text-container">
          <div class="funds-checkmark-container">
            <svg
              class="funds-checkmark"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 52 52"
            >
              <circle
                class="funds-checkmark-circle"
                cx="26"
                cy="26"
                r="25"
                fill="none"
              />
              <path
                class="funds-checkmark-check"
                fill="none"
                d="M14.1 27.2l7.1 7.2 16.7-16.8"
              />
            </svg>

            <div class="funds-display-on-ie">
              <svg
                class="funds-ie-checkmark"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 52 52"
              >
                <circle
                  class="funds-ie-checkmark-circle"
                  cx="26"
                  cy="26"
                  r="25"
                  fill="none"
                />
                <path
                  class="funds-ie-checkmark-check"
                  fill="none"
                  d="M14.1 27.2l7.1 7.2 16.7-16.8"
                />
              </svg>
            </div>
          </div>

          <h1 class="funds-success-done-text">Well done !</h1>
        </div>

        <div class="funds-success-message">
          <h2>Your payment has been validated successfully</h2>
          <p>Please go to the next step</p>
        </div>
      </div>
    </div>
  </div>
</template>



<script>
import Swal from "sweetalert2";
import loadingPage from "./loadPage.vue";

export default {
  components: { loadingPage },
  data() {
    return {
      loading: true,
      orderInfo: {
        order_id: "",
        user_id: "",
        paymentStatus: "",
        paymentId: "",
        paymentRef: "",
        amount: "",
        orderItemStatus: "",
      },
      total: 0,

      user: {
        user_id: null,
        token: null,
      },

      selectCardTypeValue: "",
      creditCard: {
        nameOfCard: "",
        cardNumber: "",
        cardExpDate: "",
        cardCvvCode: "",
        cardType: "",
        user_id: "",
        nameOfCard: "",
        order_id: "",
      },

      errors: {
        nameOfCard: "",
        cardNumber: "",
        cardExpDate: "",
        cardCvvCode: "",
        cardType: "",
        user_id: "",
        nameOfCard: "",
      },

      cart: [],
      paid: false,

      addNewCreditCard: false,

      creditCards: [],
      addNewCardState: false,

      state:false,

      getCartItemsState : false,



      creditCardDashedNumber : [],
    };
  },


  mounted() {
    this.user.user_id = localStorage.getItem("user_id");
    this.user.token = localStorage.getItem("token");
    this.creditCard.user_id = localStorage.getItem("user_id");
    this.getCartItem();
    this.todo();

    this.getTotal();
    this.getCreditCardForUser();
  },
  methods: {



      todo: function () {
      this.intervalid1 = setInterval(
        function () {
          let x = localStorage.getItem("paymentStep");
          if(x==2 && !this.getCartItemsState){
          this.getCartItem();
          this.getCartItemsState=true;
          }else if (x!=2){
            this.getCartItemsState=false;
          }
        }.bind(this),
        200
      );
    },

      resetCreditCard(){
          this.creditCard= {
        nameOfCard: "",
        cardNumber: "",
        cardExpDate: "",
        cardCvvCode: "",
        cardType: "",
        nameOfCard: "",
      }

          this.creditCard.user_id = localStorage.getItem("user_id");

},

    verifyMe() {
      //console.log('3333éé"é"é332333VERRRRT');
      //   addNewCardState  ? addNewCreditCard=false : setStepCart()
      if (this.addNewCardState) {
        this.setStepCart();
        this.addNewCreditCard = true;
        localStorage.setItem("userAddress_id", "");
      } else {
        this.addNewCardState = false;
        this.addNewCreditCard = false;
        localStorage.setItem("userAddress_id", "");
      }
    },
    setStepCart() {
      localStorage.setItem("paymentStep", 2);
      localStorage.setItem("userAddress_id", "");
    },
    setCreditCardInfo(x) {
      //console.log(this.creditCards[x]);
      this.creditCard = this.creditCards[x];

      let that = this;

      Swal.fire({
        imageUrl: "/icons/cvv.png",
        customClass: "swal-wide",
        confirmButtonColor: "#0370c8",
        confirmButtonClass: " px-4 py-2 rounded-pill ",

        imageWidth: 150,
        imageHeight: 150,
        html: `<input type="text" id="cvvValue" class="swal2-input" maxlength="5">`,

        preConfirm: () => {
          const cvvValue = Swal.getPopup().querySelector("#cvvValue").value;
          return cvvValue;
        },
      }).then(function (result) {
        if (result.value) {
         // console.log("ccvvvvv", cvvValue.value);
          if (parseInt(cvvValue.value) == that.creditCard.cardCvvCode) {
            that.addCreditCard();
          } else {
            Swal.fire({
              icon: "error",
              text: "Sorry wrong CVV please try again !",
              confirmButtonColor: "#0370c8",
            });
          }
        }
      });
    },

    updateValue2($event) {
      let value = $event.target.value;
      //console.log(value)
      if(value>12 && (String(value).length)==2 ){
          this.creditCard.cardExpDate = 12;
      }
      if ((String(value).length + 1) % 5 == 0 && String(value).length != 0 && this.state==false ) {
        this.creditCard.cardExpDate= this.creditCard.cardExpDate.slice(0, 2) +
          " / 20" +
          this.creditCard.cardExpDate.slice(2, 4);
          this.state=true;
      }
if ((String(value).length < 4)){
              this.state=false;

}

    },
    // updateValue($event) {
    //   let value = $event.target.value;
    // let length = (this.creditCard.cardNumber.length);

    //   if ((String(value).split(" - ").join("").length ) % 4 == 0 && !this.state  && (String(value).split(" - ").join("").length ) !=16  && this.creditCard.cardNumber.charAt(length-1) != ' ' && this.creditCard.cardNumber.charAt(length-1) != '-') {
    //     this.creditCard.cardNumber += " - ";
    //     this.state=true;
    //   }







    //   if ((String(value).length)  == 6 ||  (String(value).length)  == 11 ||(String(value).length)  == 16 ||(String(value).length)  == 20 ||(String(value).length)  == 0  )  {

    //      this.state=false;
    // }
    // },

    onlyNumber($event) {
     // console.log($event.keyCode); //keyCodes value
      let keyCode = $event.keyCode ? $event.keyCode : $event.which;

      if ((keyCode < 48 || keyCode > 57) && keyCode !== 46) {
        // 46 is dot
        $event.preventDefault();
      }
    },

    getCreditCardForUser() {
      let config = {
        headers: {
          Authorization: `Bearer ${this.user.token}`,
        },
      };

      axios
        .post("api/getCreditCardUser", this.user, config)
        .then((response) => {
          if (response.data.status == "found") {
            this.addNewCreditCard = false;
                            console.log(' rtezrtzerrtt')

            response.data.creditCard.forEach(element => {
              this.creditCards.push(element);
            });


             this.creditCards.forEach(element => {
                this.creditCardDashedNumber.push(element.cardNumber)
            });

            this.creditCardDashedNumber.forEach((element , key) => {
                this.creditCardDashedNumber[key] = element.replace(/\s+/g, '');
                 this.creditCardDashedNumber[key] = this.creditCardDashedNumber[key].replace(/.(?=.{4})/g, "*");
                     this.creditCardDashedNumber[key] = this.creditCardDashedNumber[key].replace(/(.{4})/g, '$1 ').trim();
            });


          } else {
            this.addNewCreditCard = true;
            this.addNewCardState = true;
          }
        });
    },

    getTotal() {
      let config = {
        headers: {
          Authorization: `Bearer ${this.user.token}`,
        },
      };

      axios
        .post("api/getCart", this.user, config)
        .then((response) => {
         // console.log("hahahaha", response);

          response.data.cartItems.forEach((element) => {
            console.log("xxxxxxx");
            this.total += element.quantity * element.price;
          });
        })
        .catch((err) => {});
    },

    passOrder() {
      let config = {
        headers: {
          Authorization: `Bearer ${this.user.token}`,
        },
      };
     // console.log("order : ", this.orderInfo);

      this.orderInfo.user_id = localStorage.getItem("user_id");
      this.orderInfo.paymentStatus = "Waiting for payment";

      axios.get("api/lastIndexInPayementTable", config).then((response) => {
        //console.log("lastPayement", response.data);
        this.orderInfo.paymentId = response.data.id;
        this.orderInfo.paymentRef =
          this.orderInfo.user_id +
          "/" +
          this.orderInfo.paymentId +
          "Sum = " +
          this.total;
        this.orderInfo.amount = 0;
        this.orderInfo.amount = this.total;
        this.orderInfo.userAddress_id = localStorage.getItem("userAddress_id");
        axios.post("api/passOrder", this.orderInfo, config).then((response) => {
          this.creditCard.order_id = response.data.order.id;
          this.orderInfo.order_id = response.data.order.id;
          console.log(response.data.order.id , 'taaaaaaaw')
          localStorage.setItem("lastOrderId", response.data.order.id);

          this.cart.forEach((element) => {
            let orderId = localStorage.getItem("lastOrderId");
            element.order_id = this.orderInfo.order_id;
            element.orderItemStatus = "Not shipped";
            element.userAddress_id = localStorage.getItem("userAddress_id");

            axios
              .post("api/addOrderItems", element, config)
              .then((response) => {
                //console.log(response);
                this.paid = true;
                setTimeout(() => {
                  localStorage.setItem("paymentStep", 4);
                }, 1000);
                //axios.post("api/cartDelete/" + element.id, this.user, config);
                localStorage.setItem("countCart", 0);
              });
          });

                axios.post('api/sendOrderConfirmationMail' , this.orderInfo , config)
                .then((response) => {

                }).catch((err) => {

                });

        });
      });
    },

    addCreditCard() {
     // console.log(this.creditCard);
      let config = {
        headers: {
          Authorization: `Bearer ${this.user.token}`,
        },
      };
      this.creditCard.order_id = localStorage.getItem("lastOrderId");

      Swal.fire({
        title: "Pay it ? ",
        icon: "info",
        showCancelButton: true,
        cancelButtonColor: "#ee4646",
        confirmButtonColor: "#0370c8",
        confirmButtonText: "Yes",

        customClass: { popup: "swal2-border-radius" },
        cancelButtonClass: " px-4 py-2 rounded-pill ",
        confirmButtonClass: " px-4 py-2 rounded-pill ",
      }).then((response) => {
        if (response.value) {
          axios
            .post("api/addCreditCard", this.creditCard, config)
            .then((response) => {
              if (response.data.status == "failed") {
                this.errors = response.data.errors;
              } else {
                this.passOrder();
                this.errors = {
                  nameOfCard: "",
                  cardNumber: "",
                  cardExpDate: "",
                  cardCvvCode: "",
                  cardType: "",
                  user_id: "",
                  nameOfCard: "",
                };
              }
            })
            .catch((err) => {});
        }
      });
    },
 detectCardType(number) {

    var re = new RegExp("^4");
    if (number.match(re) != null){
         console.log('visa')
         this.creditCard.cardType="Visa"
        return "Visa";
    }

    // Mastercard
    // Updated for Mastercard 2017 BINs expansion

     if (/^(5[1-5][0-9]{14}|2(22[1-9][0-9]{12}|2[3-9][0-9]{13}|[3-6][0-9]{14}|7[0-1][0-9]{13}|720[0-9]{12}))$/.test(number))
     {
         console.log('MasterCard')
          this.creditCard.cardType="Mastercard"
         return "Mastercard";
     }
    // AMEX
    re = new RegExp("^3[47]");
    if (number.match(re) != null)
     {
         console.log('AMEX')
          this.creditCard.cardType="AMEX"
        return "AMEX";
     }

    // Discover
    re = new RegExp("^(6011|622(12[6-9]|1[3-9][0-9]|[2-8][0-9]{2}|9[0-1][0-9]|92[0-5]|64[4-9])|65)");
    if (number.match(re) != null)
    {
         console.log('Discover')
          this.creditCard.cardType="Discover"
        return "Discover";
}
    // Diners
    re = new RegExp("^36");
    if (number.match(re) != null)
     {
          this.creditCard.cardType="Diners"
         console.log('Diners')
        return "Diners";
     }

    // Diners - Carte Blanche
    re = new RegExp("^30[0-5]");
    if (number.match(re) != null)
      {
           this.creditCard.cardType="Diners CB"
         console.log('Diners - Carte Blanche')
        return "Diners - Carte Blanche";
      }
    // JCB
    re = new RegExp("^35(2[89]|[3-8][0-9])");
    if (number.match(re) != null)
 {
      this.creditCard.cardType="JCB"
         console.log('JCB')
        return "JCB";
 }
    // Visa Electron
    re = new RegExp("^(4026|417500|4508|4844|491(3|7))");
    if (number.match(re) != null)
    {
         this.creditCard.cardType="Visa Electron"
         console.log('Visa Electron')
        return "Visa Electron";
    }


    this.creditCard.cardType="CB"
    return "CB";



},
    selectCardType(ch) {
      console.log(ch);
      this.selectCardTypeValue = ch;
      this.creditCard.cardType = ch;
    },


    getCartItem() {
      this.cart = [];

      let config = {
        headers: {
          Authorization: `Bearer ${this.user.token}`,
        },
      };

      axios.post("api/getCart", this.user, config).then((response) => {
        //console.log(response.data.cartItems);
        response.data.cartItems.forEach((element) => {
          this.cart.push(element);
        });

       // console.log(this.cart);
      });
    },
  },
};
</script>



<style>
.amex {
  background: url("data:image/png;base64,")
    #fff;
}

.visa {
  background: url("data:image/png;base64,")
    #fff;
}

.mastercard {
  background: url("data:image/png;base64,")
    #fff;
}

.discover {
  background: url("data:image/png;base64,")
    #fff;
}

.expiry-date-group {
  float: left;
  width: 50%;
}

.expiry-date-group input {
  width: calc(100% + 1px);
  border-top-right-radius: 0;
  border-bottom-right-radius: 0;
}

.expiry-date-group input:focus {
  position: relative;
  z-index: 10;
}

.security-code-group {
  float: right;
  width: 50%;
}

.security-code-group input {
  border-top-left-radius: 0;
  border-bottom-left-radius: 0;
}

.zip-code-group {
  clear: both;
}

#PayButton {
  outline: 0 !important;
  height: 42px;
  font-size: 16px;
  background-color: #54c7c3 !important;
  border: none;
}

#PayButton:hover {
  background-color: #6dcecb !important;
}

#PayButton:active {
  background-color: #4fbcb9 !important;
}

#PayButton:disabled {
  background: rgba(84, 199, 195, 0.5) !important;
  color: #fff !important;
}

#Checkout {
  z-index: 100001;

  width: 50%;
  min-width: 300px;
  height: 100%;
  min-height: 100%;
  background: 0 0 #ffffff;
  border-radius: 8px;
  border: 1px solid #dedede;
  margin-left: auto;
  margin-right: auto;
  display: block;
}

#Checkout > h1 {
  margin: 0;
  padding: 20px;
  text-align: center;
  background: #eef2f4;
  color: #5d6f78;
  font-size: 24px;
  font-weight: 300;
  border-bottom: 1px solid #dedede;
  border-top-left-radius: 8px;
  border-top-right-radius: 8px;
}

#Checkout > .formDiv {
  margin: 0 25px 25px;
}

#Checkout label {
  color: #46545c;
  margin-bottom: 2px;
}

#Checkout .input-container {
  position: relative;
}

#Checkout .input-container input {
  padding-right: 25px;
}

#Checkout .input-container > i,
#Checkout a[role="button"] {
  color: #d3d3d3;
  width: 25px;
  height: 30px;
  line-height: 30px;
  font-size: 16px;
  position: absolute;
  top: 2px;
  right: 2px;
  cursor: pointer;
  text-align: center;
}

#Checkout .input-container > i:hover,
#Checkout a[role="button"]:hover {
  color: #777;
}
.amount-placeholder {
  font-size: 20px;
  height: 34px;
}

#Checkout .amount-placeholder > button {
  float: right;
  width: 60px;
}

#Checkout .amount-placeholder > span {
  line-height: 34px;
}

#Checkout .card-row {
  text-align: center;
  margin: 20px 25px 10px;
}

#Checkout .card-row span {
  width: 48px;
  height: 30px;
  margin-right: 3px;
  background-repeat: no-repeat;
  display: inline-block;
  background-size: contain;
}

#Checkout .card-image {
  background-repeat: no-repeat;
  padding-right: 50px;
  background-position: right 2px center;
  background-size: auto 90%;
}

#Checkout .cvc-preview-container {
  overflow: hidden;
}

#Checkout .cvc-preview-container.two-card div {
  width: 48%;
  height: 80px;
}

#Checkout .cvc-preview-container.two-card div.amex-cvc-preview {
  float: right;
}

.cvc-preview-container.two-card div.visa-mc-dis-cvc-preview {
  float: left;
}

.cvc-preview-container div {
  height: 160px;
}

.amex-cvc-preview {
  background: url("data:image/png;base64,")
    center center/contain no-repeat;
}

.visa-mc-dis-cvc-preview {
  background: url("data:image/png;base64,")
    center center/contain no-repeat;
}

.submit-button-lock {
  height: 20px;
  margin-top: -2px;
  margin-right: 7px;
  vertical-align: middle;
  background: url("data:image/png;base64,")
    center center/contain no-repeat;
  width: 14px;
  display: inline-block;
}

#Checkout .align-middle {
  vertical-align: middle;
}

#Checkout input {
  box-shadow: none !important;
}

#Checkout input:focus {
  border-color: #b0e5e3 !important;
  background-color: #eef9f9 !important;
}
</style>
